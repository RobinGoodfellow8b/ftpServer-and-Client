 ...
 * as well as (B) to exercise initializing TCP connections, and (C), to 
 * exercise using various data-streams. 
 *  
    ...
    public void run(){   
        ...
        while (stayAlive) {
            ...
            while (tokens.hasMoreTokens()) {
                ...
                * When the server receives a LIST command, it establishes a
                * link with the data port, sets up the I/O streams, then
                * retrieves the contents of its directory, before sending
                * it back to the client in a formatted string.
                *
                **********************************************************/
                if (commandFromClient.equals("LIST")) {
                    ...
                    try {
                       ...
                       inFromClient_Data = dataConnection.getInputStream();
                       outToClient_Data = dataConnection.getOutputStream();
                ...
                * When the server receives a RETR command, it establishes a
                * link with the data port, sets up the I/O streams, then
                * retrieves the file the client asked for, converts it into
                * bytes, then outputs it back to the client to be converted
                * back into a file.
                *
                **********************************************************/
                ...
                * When the server receives a STOR command, it establishes a
                * link with the data port, sets up the I/O streams, then
                * receives an incoming file sent by the client as a series
                * of bytes, then writes the bytes to a file, creating the 
                ...
                * When the server receives a QUIT command, it changes the 
                * boolean variable "stayAlive" to false and breaks out of 
                * the loop, resulting in the while loop responsible for 
                * maintaining a connection to the client being terminated.
                *
                **********************************************************/
                ...
                * The client normally filters out all invalid commands from
                * ever reaching the server. As such, this should never be
                * called in standard operation. If it is, then someone has 
                * manipulated the client to send unwelcome data; thus, the
                * connection auto-terminates in self-defense.
                *
                **********************************************************/
                } else {
